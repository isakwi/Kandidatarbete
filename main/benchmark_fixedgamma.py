...
qb1 = qbc.Qubit(3, [G1qb1, Gphiqb1, c],
                -229e6 * 2 * pi)  # In other parts of the program we work linearly, right? Should be angular, So this is correct. Input in .csv is linear
...
for problem in ['a', 'b', 'c', 'd', 'e']:
    ...
    # t00 = time.time()
    # t0 = time.time()
    for i in range(gamma_ind, gamma_ind + 1):  # gamma_resolution):
        gamma = gamma_vec[i]
        # if i > 0:
        #    t = time.time()
        ...
        for j in range(0, beta_resolution):
            ...
            args = {"steps": steps, "c_ops": c_ops, "psi0": psi0, "Qblist": qblist, "t_max": tmax, "ntraj": ntraj,
                    "StoreTimeDynamics": False, "zz_mat": zz_mat}
            args["e_ops_inp"] = []
    ...
    if betaplot:
        ...
        if storeData:
            with np.printoptions(threshold=np.inf):
                file = open("plotdata_2betaplot_" + problem + ".txt", "w+")  # Seems to work
